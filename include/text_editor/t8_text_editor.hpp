...
namespace t8::text_editor
{
    ...
    private:
        ...
        void update_lines_erase(size_t start, size_t end)
        {
            const auto first = std::lower_bound(line_starts.begin(), line_starts.end(), start);
            const auto last = std::lower_bound(line_starts.begin(), line_starts.end(), end);
        ...
        void update_lines_insert(size_t start, size_t end)
        {
            const auto count = end - start;
            auto it = std::lower_bound(line_starts.begin(), line_starts.end(), start);

            for (auto i = start; i < end; i += 1)
            {
                if (buf[i] == '\n')
                {
                    it = line_starts.insert(it, i + 1);
                    it += 1;
                }
            }
        ...
        bool delete_selection()
        {
            ...
            update_lines_erase(sel.start(), sel.end());
    ...
    public:
        void insert(const std::string &text)
        {
            ...
            buf.insert(text);
            sel.collapse(buf.cursor());
            update_lines_insert(cursor, cursor + text.size());
        }
        ...
        void erase_before()
        {
            if (!delete_selection() && buf.cursor() > 0)
            {
                ...
                update_lines_erase(buf.cursor() - 1, buf.cursor());
                buf.erase_before(1);
            }
        ...
        void erase_after()
        {
            if (!delete_selection() && buf.cursor() < buf.size())
            {
                ...
                update_lines_erase(buf.cursor(), buf.cursor() + 1);
                buf.erase_after(1);
            }
            ...
            if (rec.can_undo())
            {
                ...
                if (o.type == EditType::Delete)
                {
                    if (o.backspace)
                    {
                        ...
                        buf.insert(o.text);
                        update_lines_insert(o.pos - o.text.size(), o.pos);
                    }
                    else
                    {
                        buf.set_cursor(o.pos);
                        buf.insert(o.text);
                        update_lines_insert(o.pos, o.pos + o.text.size());
                    }
                }
                ...
                if (o.type == EditType::Insert)
                {
                    ...
                    update_lines_erase(o.pos, o.pos + o.text.size());
                }
            ...
            if (rec.can_redo())
            {
                ...
                if (o.type == EditType::Delete)
                {
                    if (o.backspace)
                    {
                        ...
                        update_lines_erase(o.pos - o.text.size(), o.pos);
                    }
                    else
                    {
                        buf.set_cursor(o.pos);
