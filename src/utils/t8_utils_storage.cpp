...
namespace t8::utils {
    ...
    bool storage_load_cart(const std::string &name) {
        ...
        for (const auto &f : fhs) {
            ...
            if (f.name == 'sprt') {
                in.read(memory()->sprite, std::min(0x2000U, f.size));
            }
            if (f.name == 'map') {
                in.read(memory()->map, std::min(0x4000U, f.size));
            }
            if (f.name == 'font') {
                in.read(memory()->custom_font, std::min(0x800U, f.size));
            }
            if (f.name == 'scrp') {
                core::context()->script = in.read_str(f.size);
            }
        }
    ...
    bool storage_save_cart(const std::string &name) {
        ...
        out.write_buf(memory()->sprite, 0x2000);
        out.write_buf(memory()->map, 0x4000);
        out.write_buf(memory()->custom_font, 0x800);
        out.write_buf(context()->script.data(), context()->script.size());
        ...
        out.write<uint32_t>(context()->script.size());
        out.write<uint32_t>(offset);
