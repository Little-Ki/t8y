...
namespace t8::scene {
    ...
    void update_script_editor() {
        ...
        if (mouse_inside(8, 11, 120, 128) || script_state.dragging) {
            ...
            const auto line_start = script_state.editor.line_start(line);
            const auto line_size = script_state.editor.line_size(line);
            const auto max_size = line == script_state.editor.line_count() - 1 ? line_size : line_size - 1;
    ...
    void script_editor_set_script(const std::string &script) {
        script_state.editor.reset(script);
    }

    std::string script_editor_get_script() {
        return script_state.editor.to_string();
    }
