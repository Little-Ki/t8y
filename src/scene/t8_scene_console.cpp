...
namespace t8::scene {
    ...
    bool console_command() {
        ...
        if (str_equals(payload[0], "run")) {
            if (console_validate(payload, 1)) {
                context()->signals.push({SIGNAL_SWAP_EXECUTOR});
                return true;
            }
        }
    ...
    void console_update() {
        ...
        if (keyboard_pressed(41)) {
            context()->signals.push({SIGNAL_SWAP_EDITOR});
            return;
        }
        ...
        if (keyboard_triggered(79)) {
            state.cursor++;
            console_sanitize_cursor();
            return;
        }

        if (keyboard_triggered(82)) {
    ...
    void console_draw() {
        ...
        for (const auto &line : state.lines) {
            if (line.prefix) {
                painter_char('>', x, y, line.color);
                x = 8;
            }

            for (const auto &ch : line.content) {
                painter_char(ch, x, y, line.color);
                x += 4;
    ...
    void console_enter() {
        context()->signals.push({SIGNAL_START_INPUT});
        painter_reset();
