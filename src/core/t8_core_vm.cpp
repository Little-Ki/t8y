...
namespace t8::core {
    ...
    int vm_call_pmem(const VMCall &call) {
        if (call.is<uint8_t>()) {
            return call.ret(
                memory()->cache[call.get<uint8_t>(1)]);
        }
        if (call.is<uint8_t, uint32_t>()) {
            memory()->cache[call.get<uint8_t>(1)] = call.get<uint32_t>(2);
        }
        return 0;
    }
    ...
    int vm_call_pal(const VMCall &call) {
        if (call.is<int, int>()) {
            painter_palette(call.get<int>(1), call.get<int>(2));
        }
        if (call.is<int>()) {
            auto p = painter_palette(call.get<int>(1));
            return call.ret(p);
        }
        return 0;
    }
    ...
    int vm_call_cls(const VMCall &call) {
        ...
        if (call.is<uint8_t>()) {
            painter_clear(call.get<uint8_t>(1));
        }
        return 0;
    }
    ...
    int vm_call_fget(const VMCall &call) {
        if (call.is<uint8_t>()) {
            return call.ret(
                painter_flags(call.get<uint8_t>(1)));
        }
        return 0;
    }
    ...
    int vm_call_spr(const VMCall &call) {
        ...
        for (auto inY = 0; inY < 8; inY++) {
            for (auto inX = 0; inX < 8; inX++) {
                auto tx = x;
                auto ty = y;
                auto color = painter_sprite(spX + tx, spY + ty);
    ...
    int vm_call_map(const VMCall &call) {
        ...
        for (auto y = t; y < b; y++) {
            for (auto x = l; x < r; x++) {
                ...
                for (auto inY = 0; inY < 8; inY++) {
                    for (auto inX = 0; inX < 8; inX++) {
                        auto color = painter_sprite(spX + inX, spY + inY);
                        painter_rect(
    ...
    int vm_call_key(const VMCall &call) {
        if (call.is<uint8_t>()) {
            return call.ret(keyboard_down(call.get<uint8_t>(1)));
        }
        return 0;
    }
    ...
    int vm_call_keyp(const VMCall &call) {
        if (call.is<uint8_t>()) {
            return call.ret(keyboard_pressed(call.get<uint8_t>(1)));
        }
        return 0;
    }
    ...
    int vm_call_log(const VMCall &call) {
        if (call.is<std::string>()) {
            context()->signals.push({SIGNAL_PRINT, call.get<std::string>(1)});
        }
        return 0;
    }

    int vm_call_time(const VMCall &call) {
        return call.ret(timer_ticks());
    }
